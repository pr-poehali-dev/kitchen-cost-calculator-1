...
function PricelistUpdateModal({ onClose }: { onClose: () => void }) {
  ...
  // Нормализация размера для сравнения
  ...
  const findMatches = (items: PriceItem[], seriesKey: string): Match[] => {
    const prefix = seriesKey + '_'; // e1_, e2_, e3_, k3_
    const result: Match[] = [];

    for (const mat of store.materials) {
      if (!mat.variants?.length) continue;
      // Берём только варианты нужной серии (id начинается с prefix)
      ...
      for (const v of seriesVariants) {
        ...
        // Сопоставление: размер + толщина + (если есть) подгиб
        const match = items.find(item => {
          ...
          // Дополнительно: сопоставление по подгибу если задан
          if (vParams && item.params) {
            const iParams = item.params.replace(/\s/g, '').toLowerCase();
            // 00/U1/05 → ищем "00" или "u1" в params прайса
