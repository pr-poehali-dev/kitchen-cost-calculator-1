  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      ...
      >
        ...
        <span className="text-[11px] font-medium text-foreground">Переменные</span>
        {open && hasTarget && (
        ...
        <Icon name={open ? 'ChevronUp' : 'ChevronDown'} size={13} className="text-[hsl(var(--text-muted))] shrink-0" />
      </button>
      ...
      {open && (
        <>
          ...
          <div className="flex gap-1.5 px-3 py-2 overflow-x-auto border-t border-border scrollbar-none">
            {VAR_GROUPS.map(g => (
              <button
                key={g.label}
          ...
          {/* Переменные */}
          <div className="p-2 flex flex-wrap gap-1 max-h-40 overflow-y-auto border-t border-border">
            {filtered.length === 0 ? (
