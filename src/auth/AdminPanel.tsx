...
export default function AdminPanel({ currentUser, token, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[hsl(220,14%,11%)] border border-border rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-fade-in">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            <Icon name="Shield" size={16} className="text-gold" />
            <span className="font-semibold">Управление пользователями</span>
            <span className="text-xs text-[hsl(var(--text-muted))] bg-[hsl(220,12%,16%)] px-2 py-0.5 rounded-full">{users.length} польз.</span>
          </div>
        ...
        <div className="flex-1 overflow-auto scrollbar-thin p-6">
          {loading ? (
            ...
            <div className="bg-[hsl(220,12%,14%)] rounded-lg border border-border overflow-hidden">
              <div className="grid text-xs uppercase tracking-wider text-[hsl(var(--text-muted))] px-4 py-2.5 border-b border-border"
              ...
              {users.map(u => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Icon name={u.role === 'admin' ? 'ShieldCheck' : 'User'} size={13}
