...
def bulk_upsert_materials(conn, user_id, materials: list):
    """Пакетная вставка/обновление материалов (для импорта)"""
    results = []
...
def bulk_delete_by_article_prefix(conn, user_id, prefix: str):
    """Удалить материалы по префиксу артикула (для очистки перед импортом)"""
    cur = conn.cursor()
...
def update_prices_batch(conn, user_id, updates: list):
    """Пакетное обновление цен по артикулу или id"""
    count = 0
    ...
    for upd in updates:
        ...
        for v in existing_variants:
            vu = next((x for x in variants if x['variantId'] == v['id']), None)
            if vu:
                old_price = v['basePrice']
...
def handler(event: dict, context) -> dict:
    ...
    GET  ?entity=manufacturers|vendors|materials  — список
    ...
    GET  ?action=all                              — получить всё сразу (manufacturers+vendors+materials)
    """
    cors = get_cors(event)
    ok = make_ok(cors)
    err = make_err(cors)
    ...
    if method == 'POST':
        ...
        with get_db() as conn:
            ...
            if action == 'delete_manufacturer':
                ...
                if not mid:
                    return err('Нет id')
                ok_flag = delete_manufacturer(conn, user_id, mid)
            ...
            if action == 'delete_vendor':
                ...
                if not vid:
                    return err('Нет id')
                ok_flag = delete_vendor(conn, user_id, vid)
                return ok({'ok': ok_flag})
            ...
            if action == 'delete_material':
                ...
                if not mid:
                    return err('Нет id')
                ok_flag = delete_material(conn, user_id, mid)
                return ok({'ok': ok_flag})

            # Пакетные операции (импорт прайсов)
            if action == 'bulk_upsert_materials':
                materials = body.get('materials', [])
                if not materials:
                    return err('Нет materials')
                results = bulk_upsert_materials(conn, user_id, materials)
                return ok({'created': len(results), 'materials': results})
            ...
            # Синхронизация всего каталога из AppState (первая миграция данных)
            if action == 'sync_all':
