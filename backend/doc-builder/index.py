...
def apply_vars(text: str, c: dict, company: dict) -> str:
    ...
    vals = {
        ...
        # ── Техпроект ───────────────────────────────────────────────────────
        'фото_проекта':          str(c.get('tech_image_url') or '').strip(),
        ...
        'менеджер_рп':           genitive_name(str(c.get('manager_name') or '').strip()) if c.get('manager_name') else '___________',
...
def build_docx(c: dict, doc_type: str, company: dict, template_settings: dict | None = None, template_blocks: list | None = None) -> bytes:
    ...
    if template_blocks:
        ...
        _style.paragraph_format.alignment    = _ALIGN.LEFT
